...
- Stocked lines get a **Use my stock** button: a small form opens with your cost and per-warehouse quantities as the basis — pick how many to quote from the shelf (capped at the solicited quantity), set markup and shipping with a live price preview, add an optional note for the bidder, and one click creates the quote already priced. No vendor is contacted. The button then shows a green **Using my stock** check and reopens the quote on click. Partially stocked? Quote what you have, and the form shows the balance to RFQ vendors for — the solicitation won't read Priced until the full quantity is covered.
