...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${AUTH_CONFIG.API_BASE_URL}/library/analytics/my-business/open-solicitations-competitors`;
    ...
    if (!response.ok) {
      console.error('[Analytics/OpenSolicitationsCompetitors] Backend error:', data);
      return NextResponse.json(
        { error: typeof data.detail === 'string' ? data.detail : 'Failed to fetch open solicitations and competitors' },
    ...
    console.error('[Analytics/OpenSolicitationsCompetitors] Error:', errorMessage);
    return NextResponse.json(
