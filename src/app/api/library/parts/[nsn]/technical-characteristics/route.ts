...
export async function GET(
  ...
  try {
    ...
    const url = `${AUTH_CONFIG.API_BASE_URL}/library/parts/${encodeURIComponent(nsn)}/technical-characteristics`;
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data.detail || 'Failed to fetch technical characteristics' },
        { status: response.status }
      );
    }

    // Return technical characteristics directly
    return NextResponse.json(data);
  } catch (error) {
    console.error('Technical characteristics error:', error);
    return NextResponse.json(
