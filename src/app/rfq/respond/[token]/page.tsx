...
export default function RfqRespondPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const createAccount = async () => {
    ...
    setAcctError(null);
    try {
      const res = await fetch(`/api/rfq/public/${encodeURIComponent(token)}/create-account`, {
        method: "POST",
      ...
      if (!res.ok || !data.success) {
        setAcctError(data.error || "Could not create account.");
        setAcctBusy(false);
        return;
      }
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const res = await fetch(`/api/rfq/public/${encodeURIComponent(token)}`);
      const data = await res.json();
      if (!res.ok) {
        setLoadError(data.error || "This RFQ link is not available.");
      } else {
        const v = data as PublicRfqView;
        setView(v);
        setLines(Object.fromEntries(v.line_items.map((li) => [li.id, { ...blankLine }])));
        // Prefill the optional signup form from the contact we addressed the
        // RFQ to (editable by the responder).
        const nameParts = (v.contact_name || "").trim().split(/\s+/).filter(Boolean);
        setAcct((prev) => ({
          ...prev,
          first_name: prev.first_name || nameParts[0] || "",
          last_name: prev.last_name || nameParts.slice(1).join(" ") || "",
          email: prev.email || v.contact_email || "",
        }));
      }
    } catch {
  ...
  const setLine = (id: number, patch: Partial<LineForm>) =>
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch(`/api/rfq/public/${encodeURIComponent(token)}/respond`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      setError("Network error submitting your quote.");
      setSubmitting(false);
    }
  };
  ...
  const decline = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/rfq/public/${encodeURIComponent(token)}/decline`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({}),
      });
  ...
  // Free-account creation card, reused on the main view (optional) and the
  // post-response confirmation screen.
  const accountCard = (
    <div className="rounded-xl border border-border p-6 space-y-3">
      <h2 className="text-sm font-semibold text-foreground">Manage your RFQs in one place</h2>
      <p className="text-xs text-muted">
        Create a free account to view and respond to RFQs from {view?.sender_company_name} and others — no charge.
      </p>
      <div>
        <label className="block text-[11px] text-muted mb-1">Your company (CAGE)</label>
        <input className={`${inputClass} bg-muted-light/60 text-muted cursor-not-allowed`} value={view?.cage_code || ""} readOnly disabled />
        <p className="text-[11px] text-muted mt-1">
          Your account is linked to this CAGE and verified against SAM.gov.
        </p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        <input className={inputClass} placeholder="First name" value={acct.first_name}
          onChange={(e) => setAcct({ ...acct, first_name: e.target.value })} />
        <input className={inputClass} placeholder="Last name" value={acct.last_name}
          onChange={(e) => setAcct({ ...acct, last_name: e.target.value })} />
      ...
      {acctError && <p className="text-xs text-error">{acctError}</p>}
      <Button
        variant="primary"
        size="sm"
        onClick={createAccount}
        disabled={acctBusy || !acct.first_name || !acct.last_name || acct.password.length < 8}
      >
        {acctBusy ? "Creating…" : "Create free account"}
      </Button>
    </div>
  );
    ...
    return (
      <div className="max-w-2xl mx-auto p-8">
        <div className="rounded-xl border border-border bg-card-bg p-8 text-center">
          <h1 className="text-lg font-semibold text-foreground mb-2">RFQ unavailable</h1>
          <p className="text-sm text-muted">{loadError || "This link is not valid."}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-2xl mx-auto p-8 space-y-6">
        <div className="rounded-xl border border-success/30 bg-success/10 p-8 text-center">
          ...
          <p className="text-sm text-muted">
            {done === "responded"
              ? `Thanks — your quote for "${view.title}" has been sent to ${view.sender_company_name}.`
              : `You've declined to quote "${view.title}". Thanks for letting ${view.sender_company_name} know.`}
          </p>
        </div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold text-foreground mt-1">{view.title}</h1>
        ...
        {view.line_items.map((li) => {
          ...
          return (
            <div key={li.id} className="rounded-xl border border-border p-4 space-y-3">
              ...
              {li.notes && <p className="text-xs text-muted">{li.notes}</p>}

              <label className="flex items-center gap-2 text-xs text-muted">
                <input
                  type="checkbox"
                  checked={f.is_no_bid}
                  onChange={(e) => setLine(li.id, { is_no_bid: e.target.checked })}
                />
                No bid on this line
              </label>
              {f.is_no_bid && (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  <div>
                    ...
                    <input type="number" min="0" step="any" className={inputClass}
                      value={f.unit_price} onChange={(e) => setLine(li.id, { unit_price: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-[11px] text-muted mb-0.5">Qty available</label>
                    <input type="number" min="0" step="any" className={inputClass}
                      value={f.quantity_available} onChange={(e) => setLine(li.id, { quantity_available: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-[11px] text-muted mb-0.5">Lead (days)</label>
                    <input type="number" min="0" className={inputClass}
                      value={f.lead_time_days} onChange={(e) => setLine(li.id, { lead_time_days: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-[11px] text-muted mb-0.5">Alt part #</label>
                    <input className={inputClass}
                      value={f.alternate_part_number} onChange={(e) => setLine(li.id, { alternate_part_number: e.target.value })} />
                  </div>
                </div>
              )}
            </div>
      ...
      <div className="flex items-center justify-between gap-3 border-t border-border pt-4">
        <Button variant="ghost" size="sm" onClick={decline} disabled={submitting}>
          Decline to quote
        </Button>
      ...
      {isAuthenticated ? (
        ...
        >
          Or create a free account to manage your RFQs →
        </button>
      )}
    </div>
  );
